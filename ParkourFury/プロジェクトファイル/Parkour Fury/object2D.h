...
public:
	...
	//座標
	void SetPos(D3DXVECTOR2 pos) { m_pos = pos; }//設定
	D3DXVECTOR2 GetPos() { return m_pos; }//取得

	//サイズ
	void SetSize(D3DXVECTOR2 size) { m_size = size; }//設定
	D3DXVECTOR2 GetSize() { return m_size; }//取得

	//移動量
	void SetMove(D3DXVECTOR2 move) { m_move = move; }//設定
	D3DXVECTOR2 GetMove() { return m_move; }//取得

	//角度
	void SetRot(D3DXVECTOR3 rot) { m_rot = rot; }//設定
	D3DXVECTOR3 GetRot() { return m_rot; }//取得

	//カラー
	void SetColor(D3DXCOLOR col) { m_col = col; }//設定
