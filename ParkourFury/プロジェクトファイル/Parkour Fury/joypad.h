...
public:
	...
	typedef struct
	{
		float afTplDiameter[static_cast<int>(STICKTYPE::STICKTYPE_MAX)];//スティックの倒し具合
		float afAngle[static_cast<int>(STICKTYPE::STICKTYPE_MAX)];//スティックの角度
		bool abAnglePress[static_cast<int>(STICKTYPE::STICKTYPE_MAX)][static_cast<int>(STICKANGLE::STICKANGLE_MAX)];//スティックの方向プレス情報
		bool abAngleTrigger[static_cast<int>(STICKTYPE::STICKTYPE_MAX)][static_cast<int>(STICKANGLE::STICKANGLE_MAX)];//スティックの方向トリガー情報
		bool abAngleRepeat[static_cast<int>(STICKTYPE::STICKTYPE_MAX)][static_cast<int>(STICKANGLE::STICKANGLE_MAX)];//スティックの方向リピート情報
		bool abAngleRelease[static_cast<int>(STICKTYPE::STICKTYPE_MAX)][static_cast<int>(STICKANGLE::STICKANGLE_MAX)];//スティックの方向リリース情報
	}STICKINPUT;
	...
	bool GetRepeat(JOYKEY key, int nSpeed);//リピート情報取得
...
private:
	...
	DWORD m_aStickCurrentTime[static_cast<int>(STICKTYPE::STICKTYPE_MAX)][static_cast<int>(STICKANGLE::STICKANGLE_MAX)];//スティックの現在の入力時間
	DWORD m_aStickExecLastTime[static_cast<int>(STICKTYPE::STICKTYPE_MAX)][static_cast<int>(STICKANGLE::STICKANGLE_MAX)];//スティックの最後にtrueを返した時間
	STICKINPUT m_Stick;//スティックの入力情報
	XINPUT_STATE m_XInput;//コントローラーの状態
	float m_fLStickAngle;//Lスティック角度
	float m_fRStickAngle;//Rスティック角度
	bool m_bAngle[static_cast<int>(STICKTYPE::STICKTYPE_MAX)][static_cast<int>(STICKANGLE::STICKANGLE_MAX)];//各スティック角度の入力フラグ
};
