...
void CEdit2D::Update()
{
	...
	while (pUI != nullptr)
	{
			...
			pUI->GetEditFlag())
		{
			...
			//半透明にする
			pUI->SetColor({ 1.0f,1.0f,1.0,0.5f });
			break;
		}
		else
		{
			//カラーを戻す
			pUI->SetColor({ 1.0f,1.0f,1.0,1.0f });
		}
...
void CEdit2D::Change(CUI* pUI)
{
	...
	//Eキーが押された場合
		...
		nType < static_cast<int>(CUI::TYPE::MAX) - 1)
	{
		//タイプ変更
		nType++;
	}
	...
	//新たにオブジェクトを生成
	CUI::Create(static_cast<CUI::TYPE>(nType), pUI->GetPos(), pUI->GetSize(), pUI->GetRot());
}
