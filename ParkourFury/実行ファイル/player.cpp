...
{
	// メンバ変数
	m_State = STATE::NEUTRAL;// 状態
	m_nCntState = 0;// 状態カウンタ
	m_nCntDash = 0;// ダッシュ中のフレームカウンタ
	m_nCntDoublePush = 0;// 2回入力フレームカウンタ
	m_nCntJump = JUMP_TIMES;// ジャンプ回数カウンタ
	m_nDecTenCT = DEC_HEAT_CT;// テンション量減少クールタイム
	m_fInertia = BASE_INERTIA;// 減衰強度
	m_fHeat = 0.0f;// ヒート量
}
...
void CPlayer::Dash(float& fDiaSpeed, float& fFirstDiaSpeed)
{
	...
	//移動キーが入力された場合
	if (CManager::GetInstance()->GetKeyboard()->GetTrigger(DIK_A)
		|| CManager::GetInstance()->GetJoypad()->GetTrigger(CInputJoypad::JOYKEY::JOYKEY_LEFT)
		|| CManager::GetInstance()->GetKeyboard()->GetTrigger(DIK_D)
		|| CManager::GetInstance()->GetJoypad()->GetTrigger(CInputJoypad::JOYKEY::JOYKEY_RIGHT))
	{
		//2回入力受付フレームカウンタが0の場合
		if (m_nCntDoublePush <= 0)
		{
...
// 落下中処理
//===========================================================================================================
void CPlayer::Falling(CBlock::CollisionParam aParam, CBlock::WallState wallstate)
{
	//ジャンプの回数が2回残っている場合
	if (m_nCntJump == JUMP_TIMES)
	{
		//残りジャンプ回数を1回にする
		m_nCntJump = JUMP_TIMES - 1;
	}
	...
	else if (m_State == STATE::WALL_SLIDE)
	{
		...
		//壁の判定は壁ずりではない場合
		if (wallstate != CBlock::WallState::SLIDE)
		{
			//落下状態にする
			m_State = STATE::FALL;
		}
...
void CPlayer::CollisionBlock()
{
	...
	//壁の判定保存用
	CBlock::WallState wallstate = CBlock::WallState::NONE;

	//壁キック判定処理
	CollisionWallKick(aParam, wallstate);
...
// 壁キック判定処理
//===========================================================================================================
void CPlayer::CollisionWallKick(CBlock::CollisionParam& aParam, CBlock::WallState& wallstate)
{
	//崖掴まり状態ではない場合
	if (m_State != STATE::CLIFF_GRAB && !GetLandingFlag())
	{
		//壁判定処理
		wallstate = CManager::GetInstance()->GetScene()->GetBlockManager()->WallKick(aParam);
	}
	...
	{
		//壁ずり
	case CBlock::WallState::SLIDE:
		//プレイヤーを壁ずり状態に設定
		m_State = STATE::WALL_SLIDE;
		break;

		//壁キック
	case CBlock::WallState::KICK:
		//プレイヤーを壁キック状態に設定
		m_State = STATE::WALL_KICK;
