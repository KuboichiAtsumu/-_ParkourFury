...
CMotionCharacter::CMotionCharacter(int nPriority) : CObjectX(nPriority)
{
	...
	m_fSpeed = 1.0f;//速度倍率
	m_fJump = 1.0f;//ジャンプ力
	m_bLanding = true;//着地判定
	m_bMotion = true;//モーションをするかどうかの判定
}
...
void CMotionCharacter::SetMotionState(int nState)
{
	...
	if (m_nState != nState)
	{
		...
		//カウントをリセット
		m_aMotion.at(m_nState).nCntKeySet = 0;//キーセット
		m_aMotion.at(m_nState).nCntFrame = 0;//フレーム
	...
	//カウントが指定のフレーム数になった場合
	if (m_aMotion.at(m_nState).nCntFrame > 
		m_aMotion.at(m_nState).aKeyset.at(m_aMotion.at(m_nState).nCntKeySet).nMaxFrame)
	{
		//フレーム数カウントを初期化
